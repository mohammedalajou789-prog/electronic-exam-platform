  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--fg)', fontFamily: '"Plus Jakarta Sans", system-ui, sans-serif' }}>
      <main style={{ padding: '32px 28px 80px' }}>
        ...
        <h1 style={{ margin: '0 0 4px', fontSize: 26, fontWeight: 800 }}>{semesterData.name}</h1>
        <p style={{ margin: '0 0 28px', fontSize: 14.5, color: 'var(--fg-muted)' }}>Select a subject to browse batches and exams</p>
        {subjects.length > 0 ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 16 }}>
            {subjects.map((subject: any) => (
              <Link key={subject.id} href={`/${yearSlug}/basic/${semSlug}/${nameToSlug(subject.name)}`} className="year-card" style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--clr-soft)', color: 'var(--clr-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
                  <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20" /><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z" /></svg>
                </div>
                <div style={{ fontSize: 15.5, fontWeight: 800, marginBottom: 6 }}>{subject.name}</div>
                <div style={{ fontSize: 12.5, color: 'var(--fg-muted)' }}>{subject.examCount} exams · {subject.questionCount} questions</div>
              </Link>
            ))}
          </div>
        ) : (
          <div style={{ borderRadius: 18, border: '1px dashed var(--bd)', padding: '64px 24px', textAlign: 'center' }}>
