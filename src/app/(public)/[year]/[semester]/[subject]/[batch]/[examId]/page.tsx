...
export default async function ExamPrepPage({ params }: PageProps) {
  ...
  return (
    <div style={{ background: 'var(--bg)', color: 'var(--fg)', fontFamily: '"Plus Jakarta Sans", system-ui, sans-serif' }}>
      <style>{`
        .prep-main { padding: 32px 28px 80px; }
      ...
      <main className="prep-main">
        ...
        <div className="prep-breadcrumb" style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', fontSize: 13, color: 'var(--fg-muted)', marginBottom: 18 }}>
          ...
          <span>›</span>
          <Link href={`/${year}/${semester}`} style={{ color: 'var(--fg-muted)', textDecoration: 'none', flexShrink: 0 }}>{subjectName}</Link>
          <span>›</span>
          <Link href={`/${year}/${semester}/${subject}`} style={{ color: 'var(--fg-muted)', textDecoration: 'none', flexShrink: 0 }}>{batchName}</Link>
          <span>›</span>
        ...
        <div className="prep-modes">
          <div className="prep-mode-card" style={{ border: '1.5px solid var(--clr-primary)', background: 'var(--bg-elev)' }}>
            ...
            <div style={{ flex: '1 1 auto', minWidth: 0 }}>
              ...
              <div className="prep-mode-desc" style={{ fontSize: 13, color: 'var(--fg-muted)', marginTop: 2 }}>
                {savedProgress
                  ? `${answeredCount} of ${exam.question_count} answered — continue from question ${savedProgress.current_question + 1}`
            ...
            {savedProgress ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                ...
                <Link href={`${basePath}/play`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '10px 20px', borderRadius: 11, background: 'var(--clr-primary)', color: 'white', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
