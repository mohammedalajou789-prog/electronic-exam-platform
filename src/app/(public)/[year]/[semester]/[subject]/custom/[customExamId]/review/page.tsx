...
export default async function CustomExamReviewPage({ params }: PageProps) {
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mb-6">
        <Breadcrumb items={[
          ...
          { label: slugToName(subject), href: `/${year}/${semester}/${subject}` },
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Custom Exam</h1>
          ...
          return (
            <div key={question.id} className="rounded-xl border border-border/60 bg-card shadow-sm overflow-hidden">
              ...
              <div className="px-5 pb-4 space-y-2">
                {choices.map(letter => {
                  const text = question[`choice_${letter}` as keyof typeof question] as string
                  if (!text) return null
                  const isCorrect = question.correct_answer === letter
                  const wrongExpl = question[`incorrect_explanation_${letter}` as keyof typeof question] as string
