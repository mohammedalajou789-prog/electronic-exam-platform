...
import { ChevronRight, SlidersHorizontal, Target, CircleCheck } from 'lucide-react'
...
const REVIEW_CSS = `
  ...
  /* ── Weak areas line ── */
  .rv-weakline { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
  .rv-weakline-label { display: inline-flex; align-items: center; gap: 7px; font-size: 13px; font-weight: 800; color: var(--fg-2); white-space: nowrap; flex-shrink: 0; }
  .rv-weakline-label svg { color: var(--clr-primary); flex-shrink: 0; }
  .rv-weakline-chips { display: flex; flex-wrap: wrap; gap: 6px; min-width: 0; }
  .rv-weakchip {
    display: inline-flex; align-items: center; gap: 8px; padding: 5px 6px 5px 12px; border-radius: 999px;
    background: var(--bg-elev); border: 1px solid var(--bd); font-size: 12.5px; font-weight: 700; color: var(--fg-2); white-space: nowrap; cursor: default;
  }
  .rv-weakchip b {
    min-width: 20px; padding: 1px 7px; border-radius: 999px; background: var(--clr-soft); color: var(--clr-ink);
    font-family: "JetBrains Mono", ui-monospace, monospace; font-size: 11px; font-weight: 600; text-align: center;
  }
  ...
  @media (max-width: 640px) {
    ...
    .rv-weakline { flex-wrap: nowrap; overflow-x: auto; scrollbar-width: none; margin: 0 -16px; padding: 0 16px; }
    .rv-weakline::-webkit-scrollbar { display: none; }
    .rv-weakline-chips { flex-wrap: nowrap; }
...
export default async function WrongQuestionsPage({
  ...
  const subjectCounts = subjects
    ...
    .sort((a, b) => b.count - a.count)
  const spec = [
    { value: wrong.length.toLocaleString(), label: 'To review', text: false },
    { value: String(subjects.length), label: subjects.length === 1 ? 'Subject' : 'Subjects', text: false },
    ...(chapterStats[0] ? [{ value: chapterStats[0].chapter, label: 'Weakest chapter', text: true }] : []),
  ]
  const basePath = '/dashboard/wrong-questions'
    ...
    <div className="rv">
        ...
        <div className="rv-wrap rv-head">
          ...
          <div className="rv-cover">
            <div className="rv-title">
              ...
              <h1 className="rv-h1 a-rise" style={stagger(1)}>Wrong Questions</h1>
              <p className="rv-lead a-rise" style={stagger(2)}>
                Every question you missed, accumulated across all attempts. Review it, understand it, then clear it.
              </p>
            </div>
              ...
              <div className="rv-spec a-rise" style={stagger(2)}>
                {spec.map(item => (
                  <div key={item.label} className="rv-spec-item">
                    <span className={`rv-spec-value${item.text ? ' rv-spec-text' : ''}`}>{item.value}</span>
      ...
      <main className="rv-wrap rv-main rv-main-full">
        <div className="rv-col">
            ...
            <div className="rv-filter a-rise" style={stagger(2)}>
              ...
              <div className="rv-rail-wrap">
                <nav className="rv-rail" aria-label="Filter by subject">
                  ...
                  >
                    All subjects
                    <span className="rv-tab-count">{wrong.length}</span>
                  </Link>
          ...
          {/* ── Weak areas — one slim line, no side box ─────────────── */}
          {chapterStats.length > 0 && (
            <div className="rv-weakline a-rise" style={stagger(3)}>
              <span className="rv-weakline-label"><Target size={14} />Weak areas</span>
              <div className="rv-weakline-chips">
                {chapterStats.slice(0, 5).map(cs => (
                  <span
                    key={cs.chapter}
                    className="rv-weakchip"
                    title={cs.lectures.length > 0 ? cs.lectures.join(' · ') : undefined}
                  >
                    {cs.chapter}
                    <b>{cs.count}</b>
                  </span>
          ...
          <div className="rv-list">
            ...
            {filtered.length === 0 ? (
              <div className="rv-empty a-rise" style={stagger(3)}>
                <span className="rv-empty-icon"><CircleCheck size={26} /></span>
                <h3>No wrong questions here</h3>
                <p>
