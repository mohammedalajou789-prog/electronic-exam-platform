...
function ResultsContent() {
  ...
  return (
    <>
      ...
      <header style={{ position:'sticky', top:0, zIndex:40, background:'var(--bg-elev)', borderBottom:'1px solid var(--bd)', backdropFilter:'blur(10px)' }}>
        <div style={{ maxWidth:1180, margin:'0 auto', padding:'14px 24px', display:'flex', alignItems:'center', gap:16 }}>
          <button onClick={handleBack} style={{ width:38, height:38, flexShrink:0, borderRadius:11, border:'1px solid var(--bd)', background:'var(--bg-soft)', color:'var(--fg)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
      ...
      <main style={{ maxWidth:900, margin:'0 auto', padding:'32px 24px 80px' }}>
        <div style={{ background:'var(--bg-elev)', border:'1px solid var(--bd)', borderRadius:20, padding:'36px 30px', boxShadow:'0 1px 3px var(--shadow)', display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center', animation:'popIn .35s ease' }}>

          <div style={{ position:'relative', width:180, height:180, margin:'8px 0 4px' }}>
            <svg width="180" height="180" viewBox="0 0 180 180">
              <circle cx="90" cy="90" r={RADIUS} fill="none" stroke="var(--bd)" strokeWidth="14"/>
              <circle cx="90" cy="90" r={RADIUS} fill="none" stroke={gaugeColor} strokeWidth="14" strokeLinecap="round" transform="rotate(-90 90 90)"
                style={{ strokeDasharray: CIRC, strokeDashoffset: gaugeOffset, transition:'stroke-dashoffset 0.8s ease' }}
              />
            </svg>
            <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
              <div style={{ fontSize:34, fontWeight:800, color:'var(--fg)', lineHeight:1 }}>{scorePct}%</div>
              <div style={{ fontSize:13, fontWeight:600, color:'var(--fg-muted)', marginTop:6 }}>Score</div>
            </div>
          </div>

          <div style={{ fontSize:21, fontWeight:800, color:'var(--fg)', marginTop:8 }}>{headline}</div>
          <div style={{ fontSize:14, color:'var(--fg-muted)', marginTop:4 }}>{examTitle}</div>

          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(110px, 1fr))', gap:12, width:'100%', margin:'26px 0 6px' }}>
          ...
          {Object.keys(chapterStats).length > 0 && (
            <div style={{ width:'100%', textAlign:'left', marginTop:10 }}>
              <div style={{ fontSize:13.5, fontWeight:700, color:'var(--fg)', marginBottom:10 }}>By chapter</div>
          ...
          <div style={{ display:'flex', gap:10, marginTop:22, width:'100%', flexWrap:'wrap' }}>
            <button onClick={handleReview} style={{ flex:'1 1 160px', padding:13, borderRadius:12, border:'none', background:'var(--primary)', color:'#fff', fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}>Review Answers</button>
            <button onClick={handleRetake} style={{ flex:'1 1 160px', padding:13, borderRadius:12, border:'1px solid var(--bd)', background:'var(--bg-soft)', color:'var(--fg)', fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}>Retake Exam</button>
            <button onClick={handleBack} style={{ flex:'1 1 160px', padding:13, borderRadius:12, border:'1px solid var(--bd)', background:'transparent', color:'var(--fg-muted)', fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}>Back to Exam Page</button>
          </div>
        </div>
      </main>
    </>
...
export default function SharedResultsPage() {
  return (
    <Suspense fallback={
      <div style={{ display:'flex', height:'60vh', alignItems:'center', justifyContent:'center', color:'var(--fg-muted)' }}>
        Loading results...
      </div>
    }>
      <ResultsContent />
